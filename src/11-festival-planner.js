...
export function createFestivalManager() {
  ...
  return {
    addFestival(name, date, type) {
      if (!name || typeof name !== "string" || typeof date !== "string" ||!validTypes.includes(type)) return -1;
      if (!dateRegex.test(date)) return -1;
    ...
    getUpcoming(currentDate, n = 3) {
      if (typeof currentDate !== "string" || !dateRegex.test(currentDate)) return [];

      return festivals.filter(f => f.date >= currentDate).sort((a, b) => a.date.localeCompare(b.date)).slice(0, n).map(f => ({ ...f }));
    },
    getCount() {
      return festivals.length;
    }
